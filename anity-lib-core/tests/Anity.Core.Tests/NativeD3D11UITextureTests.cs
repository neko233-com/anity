...
public sealed class NativeD3D11UITextureTests
{
    ...
    public void AlphaTextureWorksWithImplicitWhiteMainTexture()
        => WithD3D11((device, canvas) =>
        {
            Texture2D alpha = MakeTexture(new[] { new Color32(128, 0, 0, 255) }, 1, 1);
            Put(canvas, 1, null, alpha, Quad(Color.white));
            var pixel = Pixel(Read(device, canvas), 32, 32);
            Assert.InRange(pixel.r, (byte)127, (byte)129);
    ...
    [Fact]
    public void ApplyReplacesSampledTextureWithoutChangingCommand()
        => WithD3D11((device, canvas) =>
        {
            Texture2D texture = MakeTexture(new[] { new Color32(255, 0, 0, 255) }, 1, 1);
            Put(canvas, 1, texture, null, Quad(Color.white));
    ...
    [Fact]
    public void D3D11TextureExposesNativeTexturePointer()
        => WithD3D11((device, _) =>
        {
            ...
            Assert.Equal(3, info.backendKind);
        });
    ...
    [Fact]
    public void DifferentTextureIdsBindSeparateShaderResourceViews()
        => WithD3D11((device, canvas) =>
        {
    ...
    [Fact]
    public void DestroyedTextureFallsBackToWhiteWithoutStaleSrvUse()
        => WithD3D11((device, canvas) =>
        {
            Texture2D texture = MakeTexture(new[] { new Color32(255, 0, 0, 255) }, 1, 1);
            Put(canvas, 1, texture, null, Quad(Color.white));
    ...
    private static void WithD3D11(Action<NativeGraphicsDevice, NativeUICanvas> action)
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) return;
        using NativeGraphicsDevice device = NativeGraphicsDevice.Create(
            GraphicsDeviceType.Direct3D11, Size, Size, false);
        using NativeUICanvas? canvas = NativeUICanvas.TryCreate();
        bool resolved = device.Handle != IntPtr.Zero &&
            device.DeviceType == GraphicsDeviceType.Direct3D11 && canvas is not null &&
            device.CreateSwapchain(Size, Size, imageCount: 3, hdr: false) &&
            device.SwapchainBackendKind == 3;
        if (Environment.GetEnvironmentVariable("ANITY_REQUIRE_D3D11") == "1")
