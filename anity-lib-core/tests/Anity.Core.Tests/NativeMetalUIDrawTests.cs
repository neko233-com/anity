...
public sealed class NativeMetalUIDrawTests
{
    ...
    public void RedQuadRasterizesCenterPixel()
        => WithMetal((device, canvas) =>
        {
            ...
            AssertPixel(pixels, 32, 32, 255, 0, 0, 255);
    ...
    public void PixelsOutsideGeometryRemainTransparent()
        => WithMetal((device, canvas) =>
        {
            ...
            byte[] pixels = SubmitAndRead(device, canvas);
            AssertPixel(pixels, 2, 2, 0, 0, 0, 0);
        });
    ...
    public void VertexBlueColorSurvivesGpuPipeline()
        => WithMetal((device, canvas) =>
        {
            ...
            byte[] pixels = SubmitAndRead(device, canvas);
            AssertPixel(pixels, 32, 32, 0, 0, 255, 255);
        });
    ...
    public void SourceAlphaBlendsAgainstTransparentTarget()
        => WithMetal((device, canvas) =>
        {
            ...
            byte[] pixels = SubmitAndRead(device, canvas);
            (byte r, byte g, byte b, byte a) = Pixel(pixels, 32, 32);
    ...
    [Fact]
    public void RectClipUsesGpuScissor()
        => WithMetal((device, canvas) =>
        {
    ...
    public void EmptySubmissionClearsPreviousFramebuffer()
        => WithMetal((device, canvas) =>
        {
            ...
            byte[] pixels = SubmitAndRead(device, canvas);
            AssertPixel(pixels, 32, 32, 0, 0, 0, 0);
    ...
    [Fact]
    public void SixFramesReuseTripleRingOnlyAfterGpuCompletion()
        => WithMetal((device, canvas) =>
        {
            Assert.True(canvas.Upsert(Desc(1), Quad(8, 8, 56, 56, 255, 0, 0, 255), QuadIndices));
            ...
            Assert.Equal(6UL, device.LastUIUploadStats.uploadGeneration);
        });
        ...
        byte r, byte g, byte b, byte a)
    {
        ...
        return new[]
        {
            Vertex(xMin, yMin, color),
            Vertex(xMax, yMin, color),
            Vertex(xMax, yMax, color),
            Vertex(xMin, yMax, color)
        };
    }
