...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform + blend-shape/visibility animation/skin/bindpose/weights 主链，支持 source take/frame-range、多 clip、raw/resampled tangent 与 Unity compression/切片顺序。Transform source curve、坐标、同步 quaternion reduction、六种 rotation order、非 XYZ gimbal 与 wrap/tie/subframe exact-bit fixture 已闭环。pre/post、rotation/scaling pivot/offset、geometric transform 的 adjusted/retained 双路径已覆盖；10 组 pivot raw Euler 与 retained position 各达到 **720/720 float exact-bit**，Pre/Post quaternion 达到 **960/960 float exact-bit**。`Renderer.m_Enabled` 已覆盖 static/import-off/raw step/resample/compression/runtime、祖先/helper topology 数值乘积与 additive/override/animated-weight 多层 bake，`NativeModelImportTests` **175/175**。其它 axis/unit/rotation-order/pivot topology、instanced/非 Mesh Renderer、material、weighted FBX SDK exact-bit、layer mute/solo、loop/root motion/additive/mask/humanoid、完整 YAML/import worker/package/cache 行为仍未闭环，故 ModelImporter 继续 🟡。 |
