...
| `anity-native` C++ | 🟡 | core/graphics/HDR/physics/audio/media/jobs/texture/transform/math/ui/model importer；model C ABI 已以 pinned ufbx 实际解析 FBX/OBJ hierarchy、mesh/submesh、单位/坐标、transform/blend animation、skin cluster/bindpose/variable weights、blend channel/frame/delta，并输出 scalar tangent/source take frame，支持 raw Maya/FBX Bezier key 与每帧中央差分 key；真实 blend/skin/animation 专项 42/42。Canvas persistent command/batch ownership 已以 native 配置 681/681 验证。material、Transform raw layer/tangent 全语义、Humanoid、Transform 层级所有权、更多物理 solver/资源导入仍待 native 化 |
