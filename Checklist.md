...
| `AnimationClip` | 🟡 | 继承Motion、length/frameRate/wrapMode、SetCurve/GetCurve、AnimationCurveBinding[]、event 主路径已具备；SampleAnimation/Animator property pose 已覆盖 Transform 与 `SkinnedMeshRenderer.blendShape.*`。FBX blend-shape 已按 Unity 2022.3 探针闭环 24 Hz deformation、中央/源 Bezier tangent、4 种 compression mode、4 档 error、source frame origin、先压缩后切片与多个 clip（专项 42/42）；Transform `resampleCurves=false` 精确保留 9 条 position/raw-Euler/scale 源 curve，重采样 quaternion 的 24×4 value、FBX tick/time 舍入及四档同步 angular reduction 已与 Unity 黑盒一致。五种非 XYZ 的 ±90° gimbal 960/960 float 已 exact-bit；10 组多圈 wrap/tie/非整帧 fixture 已保持 24 帧共享网格，并按 branch-aware user/Clamped Auto tangent、tie residue 与 Unroll signed-zero 达到 960/960 float exact-bit，删除 `2e-5` 临时门禁。通用组件 curve、pre/post rotation、多 layer、其它 tangent mode、root-motion/Player A/B 未闭环 |
...
| `AnimatorControllerLayer` | 🟡 | generic Transform 与 blend-shape float property 已接通 Override/reference-pose Additive、weight 与 AvatarMask exact-path 过滤；Transform/float additive reference 在 BlendTree 内独立合成。Humanoid muscle/IK、通用 property、synced layer、write defaults、root motion 与正式 2022.3.61f1 A/B 未闭环 |
...
| `Animator` | 🟡 | 参数、Play/CrossFade/状态推进已有主路径；generic layer 以 native Transform pose graph + 托管 float property pose 合成 Override/Additive、mask、crossfade 与 BlendTree，不再 last-clip-wins；真实 FBX blendShape clip 的直接播放、Override 与 additive reference 已测。Humanoid muscle/IK pass、通用 property、root motion、write defaults、synced layers、Playables 及完整公开面/逐帧 A/B 仍未闭环 |
...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | `AssetPostprocessor` 已按本机 Unity 2022.3.51f1 反射修正为可构造 class，公开面仅保留四属性、order/version 与四个日志重载；所有 callback 改为派生类按名称反射分发，支持 private/public instance preprocess、static 4/5 参数 batch、preprocess order 与 batch 不受该 order 影响的官方规则、上下文/importer 注入及异常事务语义，错误签名/实例 batch 被忽略，专项 **17/17**。`AssetImportContext` 已迁入 `UnityEditor.AssetImporters`。ModelImporter 已通过 `anity-native` + pinned ufbx `v0.23.0` 实际解码 FBX/OBJ hierarchy、米制 TRS、indexed Mesh/submesh、transform/blend-shape AnimationClip curves、skin cluster/bindpose/variable bone weights 与 blend shape frame/delta；blend-shape source take `1..120`、first/last frame、多 clip、中央/Bezier tangent、4 种 compression mode、4 档 error、raw curves 与先压缩后切片已进入真实资源装配，skinned/blend fixture suite **42/42**。Transform quaternion 现按 FBX tick/evaluator/normalize 浮点顺序生成 Unity 同值的 24 帧源曲线，并以联合 angular error 同步压缩四分量；五种非 XYZ 的 gimbal 与多圈 wrap/tie/subframe 已分别达到 960/960 exact-bit，`NativeModelImportTests` **92/92**。materials/animations/meshes/tangent/clip/humanDescription/avatarSetup、root-motion name 与 source Avatar 常用 Unity YAML 已双向读写；HumanBone/SkeletonBone mapping 支持增删、空/null、跨 session、转义及未知字段保留。material extraction、pre/post rotation/pivot/multi-layer 与 constant/stepped/weighted/broken tangent、完整 loop/root motion/Mecanim/humanoid、package/cache/type-tree/fileID、RunBefore/RunAfter attributes、完整 importer/platform settings 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
...
| `anity-native` C++ | 🟡 | core/graphics/HDR/physics/audio/media/jobs/texture/transform/math/ui/model importer；model C ABI 已以 pinned ufbx 实际解析 FBX/OBJ hierarchy、mesh/submesh、单位/坐标、transform/blend animation、skin cluster/bindpose/variable weights、blend channel/frame/delta，并输出 scalar tangent/source take frame，支持 raw Maya/FBX Bezier key 与每帧中央差分 key；FBX quaternion 重采样复刻 legacy tick、KFCurve float/double 阶段、FbxAMatrix/GetQ、非收缩 normalize 与 hemisphere continuity。非整帧 Euler key 现按 Transform 共享网格补足 24 sample，baked lookup 使用相对 take 时间；10 组 wrap/tie/subframe 已实现 alternate-branch user/Clamped Auto handle、identity residue 与 half-turn signed-zero，10/10 fixture、960/960 float exact-bit。Canvas persistent command/batch ownership已以 native 配置 681/681 验证。material、其它 Transform layer/tangent 全语义、Humanoid、Transform 层级所有权、更多物理 solver/资源导入仍待 native 化 |
...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform + blend-shape animation/skin/bindpose/variable weights/blend-shape 主链，支持 deformation-percent、source take frame、frame-range、同 take 多 clip、raw/resampled tangent 与 Unity imported curve compression/切片顺序。Transform 原始 position/Euler/scale 源 key、Unity FBX 坐标转换、XYZ 24×4 quaternion value 与四档同步 angular reduction已闭环；六种 rotation order 已实现 axis/parity/matrix/等价 XYZ/MatrixConverter Hermite 主链，五种非 XYZ 的 ±90° A/B 共 960/960 Unity float exact-bit。新增 10 组 720°/1080° wrap、±180° tie 与 13.5/14.5 source key fixture；非整帧路径补足 24 个共享 Transform sample，并完成相对 take baked lookup、float destination key、branch-aware user/Clamped Auto tangent、identity residue、signed-zero 与 hemisphere continuity，极值组 10/10 fixture、960/960 float exact-bit，聚焦 25/25、`NativeModelImportTests` 92/92。material、pre/post rotation/pivot、多 layer/其它 tangent mode、loop/root motion/additive/mask/humanoid、完整 YAML/import worker/package/cache 行为仍未闭环，故 ModelImporter 继续 🟡。 |
