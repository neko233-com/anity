...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | AssetImporter(assetPath/importSettingsMissing/SaveAndReimport)、TextureImporter(textureType/filterMode/compressionQuality)、ModelImporter 与 AssetPostprocessor 常用回调已接入真实项目资源管线；`.fbx`/`.obj`/`.dae`/`.blend`/`.3ds`/`.dxf` 稳定登记为 ModelImporter。materials/animations/meshes/tangent/clip/humanDescription/avatarSetup、root-motion name 与 source Avatar 常用 Unity YAML 已双向读写；HumanBone/SkeletonBone mapping 支持增删、空/null、跨 session、转义及未知字段保留，Model YAML **107/107**、humanoid/root-motion/source 组合 **174/174**，统一 Core 回归 **2909/2909**。root-motion rotation、material/avatar remap、platform model settings、完整 importer YAML/type tree 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；公开面与完整 Unity YAML/import worker 行为仍未全部闭环。 |
...
12. 🟡 动画 Avatar：HumanDescription/HumanBone/HumanLimit/SkeletonBone、Avatar/AvatarBuilder 及 AvatarMask/AvatarMaskBodyPart 的 Unity 2022.3.51f1 预备反射公开面已精确；Human/Generic AvatarBuilder 已接 `anity-native` hierarchy/rest-pose/mapping/required-bone/root-motion-name validation（23 项）；AvatarMask native body/path 状态（17 项）已进入 generic Animator layer 的 native Override/reference-pose Additive、weight、crossfade 与 exact-path mask 合成（25 项）；HumanBone/SkeletonBone、motionNodeName/sourceAvatar 主路径已双向读写。ModelImporter decoded hierarchy/additive asset、Humanoid muscle/finger/IK mask、T-pose/humanScale/retargeting/root-motion rotation、RawAvatar 清理与 2022.3.61f1 A/B 仍未全量闭环
