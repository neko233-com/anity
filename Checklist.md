...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | `AssetPostprocessor` 已按本机 Unity 2022.3.51f1 反射修正为可构造 class，公开面仅保留四属性、order/version 与四个日志重载；所有 callback 改为派生类按名称反射分发，支持 private/public instance preprocess、static 4/5 参数 batch、preprocess order 与 batch 不受该 order 影响的官方规则、上下文/importer 注入及异常事务语义，错误签名/实例 batch 被忽略，专项 **17/17**。`AssetImportContext` 已迁入 `UnityEditor.AssetImporters`；`.fbx`/`.obj`/`.dae`/`.blend`/`.3ds`/`.dxf` 稳定登记为 ModelImporter。materials/animations/meshes/tangent/clip/humanDescription/avatarSetup、root-motion name 与 source Avatar 常用 Unity YAML 已双向读写；HumanBone/SkeletonBone mapping 支持增删、空/null、跨 session、转义及未知字段保留，Model YAML **107/107**、humanoid/root-motion/source 组合 **174/174**。真实模型/动画解码、专用 postprocess 消息、batch RunBefore/RunAfter dependency attributes、AssetImportContext artifact/GUID 完整面、root-motion rotation、material/avatar remap、platform model settings、完整 importer YAML/type tree 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
