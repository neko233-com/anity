...
| `AnimatorControllerLayer` | 🟡 | generic Transform curve 已按 Unity 2022.3.51f1 batchmode 接通 native Override/reference-pose Additive、weight 与 AvatarMask exact-path 过滤；Humanoid muscle/IK、synced layer、write defaults、root motion 与正式 2022.3.61f1 A/B 未闭环 |
...
| `Animator` | 🟡 | 参数、Play/CrossFade/状态推进已有主路径；generic layer 现以 native pose graph 合成 Override/Additive、mask、crossfade 与 BlendTree，不再 last-clip-wins，25 项 native-required 用例覆盖含 NaN/Infinity/out-of-range weight 的官方边界。Humanoid muscle/IK pass、root motion、write defaults、synced layers、Playables 及完整公开面/逐帧 A/B 仍未闭环 |
...
| `Avatar` / `AvatarMask` | 🟡 | `HumanDescription` / `HumanBone` / `HumanLimit` / `SkeletonBone`、`Avatar` / `AvatarBuilder` 及 `AvatarMask` / `AvatarMaskBodyPart` 已按本机 Unity 2022.3.51f1 预备反射做到对应公开面与 native metadata 一致；AvatarBuilder 已接 native hierarchy/rest-pose/mapping validation（23 项），AvatarMask 已接 native body/path 状态（17 项）并被 generic Animator layer 的 native pose graph 消费（25 项）。HumanBone/SkeletonBone importer YAML、`motionNodeName` 与 GUID/fileID source Avatar 已双向持久化；错误额外 `RawAvatar` 已随 AssetPostprocessor 精确公开面迁移移除。Humanoid muscle/finger/IK mask、root-motion rotation、retargeting 与 Unity 2022.3.61f1 A/B 尚未闭环，故不得标完整。 |
| `HumanBodyBones` | ✅ | 枚举 |
...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | `AssetPostprocessor` 已按本机 Unity 2022.3.51f1 反射修正为可构造 class，公开面仅保留四属性、order/version 与四个日志重载；所有 callback 改为派生类按名称反射分发，支持 private/public instance preprocess、static 4/5 参数 batch、preprocess order 与 batch 不受该 order 影响的官方规则、上下文/importer 注入及异常事务语义，错误签名/实例 batch 被忽略，专项 **17/17**。`AssetImportContext` 已迁入 `UnityEditor.AssetImporters`。ModelImporter 已通过 `anity-native` + pinned ufbx `v0.23.0` 实际解码 FBX/OBJ hierarchy、米制 TRS、indexed Mesh/submesh 和 transform AnimationClip curves；有效模型主资源现为 GameObject 而非 TextAsset，`fileScale`/`defaultClipAnimations`、损坏重导入保留、五阶段 model/animation callback 及 `OnPreprocessAnimation` 设置消费已有真实 fixture **14/14**。materials/animations/meshes/tangent/clip/humanDescription/avatarSetup、root-motion name 与 source Avatar 常用 Unity YAML 已双向读写；HumanBone/SkeletonBone mapping 支持增删、空/null、跨 session、转义及未知字段保留。skin/blend shape/material extraction、完整 clip slicing/Mecanim/humanoid、package/cache/type-tree/fileID、RunBefore/RunAfter attributes、AssetImportContext artifact/GUID、完整 importer/platform settings 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
...
| `anity-native` C++ | 🟡 | core/graphics/HDR/physics/audio/media/jobs/texture/transform/math/ui/model importer；model C ABI 已以 pinned ufbx 实际解析 FBX/OBJ hierarchy、mesh/submesh、单位/坐标与 transform animation，真实 fixture 14/14。Canvas persistent command/batch ownership 已以 native 配置 681/681 验证。完整 skinned/blend/material model path、Transform 层级所有权、更多物理 solver/资源导入仍待 native 化 |
...
| Native 热路径 | 🟡 | CCD TOI / 2D SAT / ConstantForce 3D/2D / Audio decode / Texture compress / Transform 仿射矩阵链 / Matrix4x4 / Canvas vertex staging、bounds、quad indices、clip-alpha visibility / FBX-OBJ parse、triangulation、index generation、animation bake 走 anity-native；完整 skinned model、Transform state、物理世界与剩余资源导入所有权仍待 native 化 |
...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform animation 主链并有真实 fixture 14/14，不再只登记/YAML 或把有效模型当 TextAsset。skin/blend/material/完整 clip 与 humanoid、公开面、完整 Unity YAML/import worker/package/cache 行为仍未全部闭环。 |
