...
| `AnimationClip` | 🟡 | 继承Motion、length/frameRate/wrapMode、SetCurve/GetCurve、AnimationCurveBinding[]、event 主路径已具备；SampleAnimation/Animator property pose 已覆盖 Transform 与 `SkinnedMeshRenderer.blendShape.*`。FBX blend-shape 已按 Unity 2022.3 探针闭环 24 Hz deformation、中央/源 Bezier tangent、4 种 compression mode、4 档 error、source frame origin、先压缩后切片与多个 clip（专项 42/42）；Transform `resampleCurves=false` 精确保留 9 条 position/raw-Euler/scale 源 curve，重采样 quaternion 的 24×4 value、FBX tick/time 舍入及四档同步 angular reduction已与 Unity 黑盒一致。五种非 XYZ 的 ±90° gimbal 与 10 组多圈 wrap/tie/非整帧分别达到 960/960 float exact-bit。新增 10 组 pre/post rotation、rotation/scaling pivot/offset 与 geometric transform 双模式 A/B：resampled 统一 24 帧 quaternion 网格，raw 按 Unity 分支输出 3/24-key position、Euler/quaternion、scale binding，新增 **20/20**、`NativeModelImportTests` **112/112**。raw Euler 尚有最大约 `1.335e-5` 差异，`Renderer.m_Enabled` visibility binding、通用组件 curve、多 layer、其它 tangent mode、root-motion/Player A/B 未闭环 |
...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | `AssetPostprocessor` 已按本机 Unity 2022.3.51f1 反射修正为可构造 class，公开面仅保留四属性、order/version 与四个日志重载；callback 以派生类名称反射分发，支持 private/public instance preprocess、static 4/5 参数 batch、官方 order、上下文/importer 注入与异常事务语义，专项 **17/17**。`AssetImportContext` 已迁入 `UnityEditor.AssetImporters`。ModelImporter 通过 `anity-native` + pinned ufbx `v0.23.0` 实际解码 FBX/OBJ hierarchy、米制 TRS、indexed Mesh/submesh、transform/blend-shape curves、skin/bindpose/variable weights 与 blend frames；blend-shape/source take/clip/compression suite **42/42**。Transform quaternion 的 tick/evaluator/联合 angular reduction、非 XYZ gimbal 与 wrap/tie/subframe exact-bit 主链已闭环；新增 10 组 pre/post rotation、rotation/scaling pivot/offset 与 geometric transform 双模式 A/B，以 adjusted/retained 双 FBX scene 对齐静态层级和 raw position，`NativeModelImportTests` **112/112**、model/skin 联合 **154/154**。常用 Unity YAML/HumanBone/SkeletonBone mapping 已双向持久化。raw Euler exact-bit、`Renderer.m_Enabled`、material extraction、多 layer 与 constant/stepped/weighted/broken tangent、完整 loop/root motion/Mecanim/humanoid、package/cache/type-tree/fileID、RunBefore/RunAfter attributes、完整 importer/platform settings 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
...
| `anity-native` C++ | 🟡 | core/graphics/HDR/physics/audio/media/jobs/texture/transform/math/ui/model importer；model C ABI 已以 pinned ufbx 实际解析 FBX/OBJ hierarchy、mesh/submesh、单位/坐标、transform/blend animation、skin/bindpose/variable weights、blend frame/delta 与 scalar tangent/source take。FBX quaternion 已复刻 legacy tick、KFCurve/MatrixConverter float/double 阶段、FbxAMatrix/GetQ、normalize、hemisphere continuity；非 XYZ gimbal 与 wrap/tie/subframe 已有 exact-bit fixture。新增 adjusted-to-rotation-pivot 静态层级、Mesh/normal/tangent/blend-delta Y/Z basis 修正，并以 retained-pivot 第二 scene 生成 Unity raw pivot position；pre/post/pivot/geometric transform 双模式 **20/20**。Canvas persistent command/batch ownership已以 native 配置 681/681 验证。raw Euler exact-bit、visibility binding、material、多 layer/其它 tangent 全语义、Humanoid、Transform 层级所有权、更多物理 solver/资源导入仍待 native 化 |
...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform + blend-shape animation/skin/bindpose/variable weights 主链，支持 deformation-percent、source take/frame-range、多 clip、raw/resampled tangent 与 Unity imported curve compression/切片顺序。Transform source curve、Unity FBX 坐标、四档同步 quaternion reduction、六种 rotation order、非 XYZ gimbal 与 wrap/tie/subframe exact-bit fixture 已闭环。新增 pre/post rotation、rotation/scaling pivot/offset 与 geometric transform 10 组双模式 A/B：adjusted pivot 静态层级、geometry basis、retained raw position 和 Unity 分支 binding/key-count 已覆盖，新增 **20/20**，`NativeModelImportTests` **112/112**。raw Euler exact-bit 与 `Renderer.m_Enabled` 尚缺；material、多 layer/其它 tangent mode、loop/root motion/additive/mask/humanoid、完整 YAML/import worker/package/cache 行为仍未闭环，故 ModelImporter 继续 🟡。 |
