...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform + blend-shape animation/skin/bindpose/variable weights/blend-shape 主链，支持 deformation-percent、source take frame、frame-range、同 take 多 clip、raw/resampled tangent 与 Unity imported curve compression/切片顺序。Transform 原始 position/Euler/scale 源 key、Unity FBX 坐标转换、XYZ 24×4 quaternion value 与四档同步 angular reduction 已闭环；六种 rotation order（含五种非 XYZ）已实现 axis/parity/matrix/等价 XYZ/MatrixConverter Hermite 主链，五种非 XYZ 共 480/480 float 逐 bit 一致，15/15 组压缩集合逐帧一致，进入 `NativeModelImportTests` **72/72**。剩余 gimbal singular tie-break、material、pre/post rotation/pivot、多 layer/其它 tangent mode、loop/root motion/additive/mask/humanoid、公开面、完整 Unity YAML/import worker/package/cache 行为仍未全部闭环。 |
