...
| `SkinnedMeshRenderer` | ✅ | sharedMesh/bones/rootBone/quality；native ModelImporter 会从真实 FBX skin cluster 或 blend deformer 建立 renderer、bindpose、最多 8 个 variable influences、共同 rootBone、localBounds 与 blend frames，blend-only BakeMesh 可直接输出 morph geometry |
...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | `AssetPostprocessor` 已按本机 Unity 2022.3.51f1 反射修正为可构造 class，公开面仅保留四属性、order/version 与四个日志重载；所有 callback 改为派生类按名称反射分发，支持 private/public instance preprocess、static 4/5 参数 batch、preprocess order 与 batch 不受该 order 影响的官方规则、上下文/importer 注入及异常事务语义，错误签名/实例 batch 被忽略，专项 **17/17**。`AssetImportContext` 已迁入 `UnityEditor.AssetImporters`。ModelImporter 已通过 `anity-native` + pinned ufbx `v0.23.0` 实际解码 FBX/OBJ hierarchy、米制 TRS、indexed Mesh/submesh、transform AnimationClip curves、skin cluster/bindpose/variable bone weights 与 blend shape frame/delta；有效模型主资源为 GameObject，skin 与纯 morph renderer、bones/rootBone/localBounds、Generic imported Avatar 均进入真实对象装配。static/animation fixture **14/14**，新增 skinned/blend fixture **13/13**，聚焦组合 **50/50**。materials/animations/meshes/tangent/clip/humanDescription/avatarSetup、root-motion name 与 source Avatar 常用 Unity YAML 已双向读写；HumanBone/SkeletonBone mapping 支持增删、空/null、跨 session、转义及未知字段保留。blend-shape animation curve、material extraction、完整 clip slicing/Mecanim/humanoid、package/cache/type-tree/fileID、RunBefore/RunAfter attributes、完整 importer/platform settings 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
...
| `anity-native` C++ | 🟡 | core/graphics/HDR/physics/audio/media/jobs/texture/transform/math/ui/model importer；model C ABI 已以 pinned ufbx 实际解析 FBX/OBJ hierarchy、mesh/submesh、单位/坐标、transform animation、skin cluster/bindpose/variable weights 与 blend channel/frame/delta，真实 static/animation fixture 14/14、skin/blend fixture 13/13。Canvas persistent command/batch ownership 已以 native 配置 681/681 验证。blend animation/material、Transform 层级所有权、更多物理 solver/资源导入仍待 native 化 |
...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform animation/skin/bindpose/variable weights/blend-shape 主链，真实 static/animation fixture 14/14、skin/blend fixture 13/13，不再只登记/YAML 或把有效模型当 TextAsset。blend animation/material/完整 clip 与 humanoid、公开面、完整 Unity YAML/import worker/package/cache 行为仍未全部闭环。 |
