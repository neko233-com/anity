...
| `AnimationClip` | 🟡 | 继承Motion、length/frameRate/wrapMode、SetCurve/GetCurve、AnimationCurveBinding[]、event 主路径已具备；SampleAnimation/Animator property pose 已覆盖 Transform 与 `SkinnedMeshRenderer.blendShape.*`，FBX 24 Hz deformation 值、first/last frame 边界与多个切片已测。通用组件 float/object-reference curve、Unity imported cubic tangent/key compression、完整 wrap/root-motion/Player A/B 未闭环 |
...
| `AssetImporter` / `AssetPostprocessor` | 🟡 | `AssetPostprocessor` 已按本机 Unity 2022.3.51f1 反射修正为可构造 class，公开面仅保留四属性、order/version 与四个日志重载；所有 callback 改为派生类按名称反射分发，支持 private/public instance preprocess、static 4/5 参数 batch、preprocess order 与 batch 不受该 order 影响的官方规则、上下文/importer 注入及异常事务语义，错误签名/实例 batch 被忽略，专项 **17/17**。`AssetImportContext` 已迁入 `UnityEditor.AssetImporters`。ModelImporter 已通过 `anity-native` + pinned ufbx `v0.23.0` 实际解码 FBX/OBJ hierarchy、米制 TRS、indexed Mesh/submesh、transform/blend-shape AnimationClip curves、skin cluster/bindpose/variable bone weights 与 blend shape frame/delta；first/last frame 与同 take 多 clip 切片已进入真实资源装配。skinned/blend fixture suite 现 **27/27**。materials/animations/meshes/tangent/clip/humanDescription/avatarSetup、root-motion name 与 source Avatar 常用 Unity YAML 已双向读写；HumanBone/SkeletonBone mapping 支持增删、空/null、跨 session、转义及未知字段保留。material extraction、Unity curve compression/tangent、完整 loop/root motion/Mecanim/humanoid、package/cache/type-tree/fileID、RunBefore/RunAfter attributes、完整 importer/platform settings 与 Unity 2022.3.61f1 A/B 未完成，故不可标为完成。 |
...
| `TextureImporter/ModelImporter/AudioImporter` | 🟡 | 见 AssetImporter；ModelImporter 已进入 native FBX/OBJ hierarchy/indexed mesh/transform + blend-shape animation/skin/bindpose/variable weights/blend-shape 主链，支持 deformation-percent 开关、frame-range 与同 take 多 clip 切片，skin/blend suite **27/27**。material、Unity curve tangent/compression、完整 loop/root motion/additive/mask 与 humanoid、公开面、完整 Unity YAML/import worker/package/cache 行为仍未全部闭环。 |
