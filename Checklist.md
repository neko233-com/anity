...
| `Avatar` / `AvatarMask` | 🟡 | `HumanDescription` / `HumanBone` / `HumanLimit` / `SkeletonBone` 已按本机 Unity 2022.3.51f1 预备反射做到四个类型公开面与 native metadata 完全一致，31 项门禁通过；HumanBone/SkeletonBone importer YAML、`motionNodeName` 与 GUID/fileID source Avatar 已双向持久化，模型导入会建立 Avatar 子资源，相关新增门禁 36/36；`AvatarBuilder.BuildHumanAvatar`、名称映射与 AvatarMask 基础行为已实现。Avatar/AvatarBuilder/AvatarMask/RawAvatar 的完整公开面、native Avatar 构建/层级校验、root-motion rotation、humanoid muscle/retargeting 与 Unity 2022.3.61f1 A/B 尚未闭环，故不得标完整。 |
| `HumanBodyBones` | ✅ | 枚举 |
...
12. 🟡 动画 Avatar：HumanDescription/HumanBone/HumanLimit/SkeletonBone 及 Avatar/AvatarBuilder 的 Unity 2022.3.51f1 预备反射公开面已精确；Human/Generic AvatarBuilder 已接 `anity-native` hierarchy/rest-pose/mapping/required-bone/root-motion-name validation，23 个 native-required 用例覆盖非法 parent、环、重复名称、非有限 pose、零/非单位 scale 与映射边界；HumanBone/SkeletonBone、motionNodeName/sourceAvatar 主路径已双向读写。ModelImporter decoded hierarchy asset build、muscle/T-pose/humanScale/retargeting/root-motion rotation、AvatarMask/RawAvatar 清理与 2022.3.61f1 A/B 仍未全量闭环
